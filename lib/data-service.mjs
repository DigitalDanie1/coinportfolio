...
export class DataService {
  ...
  async dexHistory(dex) {
    ...
    this.historyBudget--;
    const r=await this.cached(key,3600000,()=>
      this.request(`https://api.geckoterminal.com/api/v2/networks/${encodeURIComponent(dex.chain)}/pools/${encodeURIComponent(dex.pair)}/ohlcv/hour?aggregate=1&limit=168`));
  ...
  async markets(coins,nested=false) {
    if(!nested)this.historyBudget=4;
