...
export class DataService {
  constructor({fetcher=fetch,cacheDir=null,env=process.env}={}) {this.fetcher=fetcher;this.cacheDir=cacheDir;this.env=env;this.cache=new Map();this.pending=new Map();this.cooldown=new Map();}
  ...
  async dexMarket(coin) {
    ...
    return {canonicalId:key,canonicalName:p.baseToken.name,canonicalSymbol:p.baseToken.symbol,current_price:price,market_cap:num(p.marketCap),fdv:num(p.fdv),total_volume:num(p.volume?.h24),liquidity:num(p.liquidity?.usd),price_change_percentage_24h:num(p.priceChange?.h24),price_change_percentage_7d_in_currency:null,image:p.info?.imageUrl||null,sparkline_in_7d:{price:[]},collectedHistory:history,chartSource:'DEX Screener · 연결 이후 수집',source:`DEX Screener · ${dex.chain}`,updatedAt:r.updatedAt,stale:r.stale,status:r.stale?'stale':price==null?'metadata-only':'live'};
  }
  async markets(coins) {
    const dexCoins=coins.filter(c=>c.dex),others=coins.filter(c=>!c.dex);
    if(dexCoins.length){
      const normalResult=others.length?this.markets(others):Promise.resolve({data:{},unresolved:[],sources:{}});
    ...
    for(let i=0;i<identities.length;i+=4) await Promise.all(identities.slice(i,i+4).map(async ({c,g,p})=>{
      if(c.cat==='stocks'){
        try {
          const r=await this.cached('stock:'+c.ticker,60000,()=>this.request('https://query1.finance.yahoo.com/v8/finance/chart/'+encodeURIComponent(c.ticker)+'?interval=1h&period1='+Math.floor((Date.now()-7*86400000)/1000)+'&period2='+Math.floor(Date.now()/1000)));
          const q=r.data.chart?.result?.[0],meta=q?.meta;
          if(!meta||meta.currency!=='USD')throw new Error('Unsupported stock quote currency');
          const history=(q.indicators?.quote?.[0]?.close||[]).map(num).filter(x=>x!=null);
          data[c.id]={status:r.stale?'stale':'live',current_price:num(meta.regularMarketPrice),price_change_percentage_24h:num(meta.regularMarketChangePercent),market_cap:null,image:null,sparkline_in_7d:{price:history},price_change_percentage_7d_in_currency:history[0]>0?(meta.regularMarketPrice/history[0]-1)*100:null,source:'Yahoo Finance · 전일 대비',canonicalName:meta.longName||meta.shortName,canonicalSymbol:meta.symbol,chartSource:'Yahoo Finance · 1시간',updatedAt:r.updatedAt,providerUpdatedAt:meta.regularMarketTime*1000,stale:r.stale};
        }catch{data[c.id]={status:'unavailable',message:'주식 시세 미수신 · USD 상장 티커 확인'};}return;
      }
  ...
  async chartSources(coin) {
    ...
    if(coin.cat==='stocks') {
      const quote=await this.cached('stock-chart:'+coin.ticker,3600000,()=>this.request('https://query1.finance.yahoo.com/v8/finance/chart/'+encodeURIComponent(coin.ticker)+'?range=1d&interval=1h'));
      const meta=quote.data.chart?.result?.[0]?.meta;
      const exchange={NMS:'NASDAQ',NGM:'NASDAQ',NCM:'NASDAQ',NYQ:'NYSE',ASE:'AMEX',PCX:'AMEX'}[meta?.exchangeName];
      if(!meta?.symbol||!exchange)return {sources:[],message:'지원되는 TradingView 거래소를 확인하지 못했습니다.'};
      const symbol=exchange+':'+meta.symbol;
      return {sources:[{type:'tradingview',symbol,label:symbol,url:'https://www.tradingview.com/chart/?symbol='+encodeURIComponent(symbol)}]};
    }
  ...
  async news(asset) {
    const query=`"${String(asset.name||asset.ticker).replace(/["<>]/g,'')}" ${asset.cat==='stocks'?'stock':'crypto'} when:7d`;
