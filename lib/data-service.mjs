...
export class DataService {
  ...
  async dexHistory(dex) {
    ...
    // Each row is [ts, o, h, l, c, v]; the close and its timestamp travel together.
    const usable=rows=>{
      if(!Array.isArray(rows))return {price:[],times:[]};
      const kept=rows.filter(x=>num(x[4])>0).reverse();
      return {price:kept.map(x=>num(x[4])),times:kept.map(x=>num(x[0])*1000)};
    };
    // Candles arrive newest-first; the chart wants oldest-first closes.
    if(fresh&&Date.now()-fresh.updatedAt<600000)return usable(fresh.data?.data?.attributes?.ohlcv_list);
    ...
    const r=await this.cached(key,600000,async()=>{
  ...
  async dexMarket(coin) {
    ...
    let candles={price:[],times:[]};
    try{candles=await this.dexHistory(dex);}catch{}
    const closes=candles.price||[];
    const change7d=closes.length>1&&closes[0]>0?(closes[closes.length-1]/closes[0]-1)*100:null;
    return {canonicalId:key,canonicalName:p.baseToken.name,canonicalSymbol:p.baseToken.symbol,current_price:price,market_cap:num(p.marketCap),fdv:num(p.fdv),total_volume:num(p.volume?.h24),liquidity:num(p.liquidity?.usd),txns24h:{buys:num(p.txns?.h24?.buys),sells:num(p.txns?.h24?.sells)},price_change_percentage_24h:num(p.priceChange?.h24),price_change_percentage_7d_in_currency:change7d,image:p.info?.imageUrl||null,sparkline_in_7d:{price:closes},sparklineTimes:candles.times||[],collectedHistory:history,chartSource:closes.length?'GeckoTerminal · 1시간':'DEX Screener · 연결 이후 수집',source:`DEX Screener · ${dex.chain}`,updatedAt:r.updatedAt,stale:r.stale,status:r.stale?'stale':price==null?'metadata-only':'live',message:price==null?'페어에 시세 없음 · 유동성 확인 필요':undefined};
  }
  ...
  async equityMarket(coin) {
    ...
    const closeRow=q.indicators?.quote?.[0]?.close||[],stamps=q.timestamp||[];
    const history=[],historyTimes=[];
    closeRow.forEach((c,i)=>{const v=num(c);if(v!=null){history.push(v);historyTimes.push(num(stamps[i])*1000);}});
    ...
    return {status:r.stale?'stale':price==null?'metadata-only':'live',current_price:price,
      ...
      market_cap:null,image:`https://financialmodelingprep.com/image-stock/${encodeURIComponent(ticker)}.png`,sparkline_in_7d:{price:history},sparklineTimes:historyTimes,
  ...
  async markets(allCoins,nested=false) {
    if(!nested)this.historyBudget=10;
    ...
    for(let i=0;i<identities.length;i+=4) await Promise.all(identities.slice(i,i+4).map(async ({c,g,p})=>{
      ...
      if(result) {
        result.status=result.stale?'stale':result.current_price==null?'metadata-only':'live';
        // A resolved id with no price anywhere usually means the asset hasn't started trading yet
        // (pre-TGE / points program) rather than a broken identity, so say that instead of a bare blank.
        if(result.status==='metadata-only' && !result.message)result.message='거래소 상장 전 · 공개 시세 없음';
        data[c.id]=result;
      }
  ...
  // Market-wide context for the header: the sentiment gauge and bitcoin's own tape.
  async pulse() {
    const [fng,btc,fx]=await Promise.allSettled([
      this.cached('fng',900000,()=>this.request('https://api.alternative.me/fng/?limit=2')),
      this.cached('gecko:btc',60000,()=>this.request('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin&sparkline=true&price_change_percentage=7d')),
      this.cached('fx:usdkrw',3600000,()=>this.request('https://open.er-api.com/v6/latest/USD'))
    ]);
    const out={updatedAt:Date.now()};
    if(fng.status==='fulfilled'){
      const rows=fng.value.data?.data||[];
      const now=rows[0],prev=rows[1];
      if(now)out.fearGreed={value:num(now.value),label:String(now.value_classification||''),
        previous:prev?num(prev.value):null,stale:fng.value.stale,
        updatedAt:num(now.timestamp)?num(now.timestamp)*1000:null};
    }
    if(btc.status==='fulfilled'){
      const d=(btc.value.data||[])[0];
      if(d)out.btc={price:num(d.current_price),change24h:num(d.price_change_percentage_24h),
        change7d:num(d.price_change_percentage_7d_in_currency),marketCap:num(d.market_cap),
        sparkline:(d.sparkline_in_7d?.price||[]).map(num).filter(x=>x!=null),stale:btc.value.stale};
    }
    if(fx.status==='fulfilled'){
      const krw=num(fx.value.data?.rates?.KRW);
      if(krw>0)out.usdKrw={rate:krw,stale:fx.value.stale};
    }
    if(!out.fearGreed&&!out.btc)throw new Error('시장 지표 미수신');
    return out;
  }
  async news(asset) {
  ...
  // One client load asks about every held/watched ticker at once; batch it into one call (bounded
  // concurrency, chunked upstream by the caller) instead of a News request per asset.
  async newsBatch(items) {
    const data={};
    for(let i=0;i<items.length;i+=4) await Promise.all(items.slice(i,i+4).map(async item=>{
      try{data[item.key]=await this.news(item);}
      catch(e){data[item.key]={status:'error',message:errorText(e)};}
    }));
    return {data};
  }
