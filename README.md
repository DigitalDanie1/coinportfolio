...
시장 시나리오는 ① 대상·기간과 핵심 판단 ② 사실·해석·반대 근거 ③ 기본·강세·약세 경로 ④ 자산별 조건부 행동 ⑤ 무효화 조건과 대응 ⑥ 점검일·실제 결과·복기의 6단계로 작성합니다. 안내 문장과 빈칸 틀은 placeholder이며 저장된 사용자 판단이나 시장 예측으로 취급하지 않습니다. 기존 `scenario.text`는 자유 메모에 그대로 보존하고 구조화 입력은 `scenario.fields`에 자동 저장합니다. JSON 백업에도 포함되며 시세·뉴스 요청에는 보내지 않습니다.
