...
차트 색상은 개별 티커가 아닌 카테고리에 대응합니다. 사용자 지정 카테고리 색상을 우선 사용하고 지정값이 없으면 카테고리별 기본색을 사용합니다. 범례는 카테고리별로 묶고 로고·티커·종목명을 함께 표시합니다. 차트 끝점에도 로고와 티커를 표시하며, 밀집된 차트는 범례에 마우스를 올리거나 키보드 포커스를 주면 해당 선과 끝점만 강조합니다. 로고 URL은 HTTPS만 허용하고 미수신·실패 시 티커 앞글자를 표시합니다.
