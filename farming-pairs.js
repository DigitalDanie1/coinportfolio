...
// OI 채굴 페어는 같은 거래소 안에서 서로 다른 자산을 묶으므로 거래소 슬롯·색 슬롯을 차지하지 않는다.
function venueFarmingPairs(){return activeFarmingPairs().filter(p=>!p.oi);}
// 거래소별로 쓰는 지갑. 페어 다리에 따로 적은 지갑이 없으면 이 값을 쓴다.
function venueWallet(name){return FARMING_VENUES.find(v=>v.key===venueKey(name))?.wallet||'';}
function shortWallet(w){return w&&w.length>14?w.slice(0,6)+'…'+w.slice(-4):w||'';}
...
function initFarmingPairs(){
  ...
  const used=new Set();
  for(const pair of venueFarmingPairs()){
...
function selectFarmingVenue(key){
  ...
  if(venueFarmingPairs().length>=MAX_FARMING_PAIRS){pairPickerMessage=`${MAX_FARMING_PAIRS}개 페어가 모두 구성되었습니다. 기존 페어를 해제해 조합을 바꿔보세요.`;renderPairBoard();return;}
  ...
  const pair={id:genId('pair'),name:first.name+' × '+venue.name,ticker:'',long:{venue:first.name,wallet:first.wallet||''},short:{venue:venue.name,wallet:venue.wallet||''},reason:'',conviction:0,strategy:PAIR_STRATEGY};
...
function pairLeg(pair,side){
  ...
  if(!saved.positionId)return {...saved,side,ticker:saved.asset||pair.ticker,manual:true};
...
function pairMetrics(pair){
  ...
  if(pair.oi){const nl=ql>0&&lm>0?ql*lm:null,ns=qs>0&&sm>0?qs*sm:null;return {long,short,net:null,imbalance:null,gross:nl&&ns?nl+ns:null,stale,status:nl&&ns?`명목가 차이 ${(Math.abs(nl-ns)/Math.max(nl,ns)*100).toFixed(1)}%`:'수량 미입력'};}
...
function renderPairBoard(){
  ...
  const venueNames=new Set(venueFarmingPairs().flatMap(p=>{const m=pairMetrics(p);return [m.long.venue,m.short.venue].filter(Boolean).map(x=>x.trim().toLowerCase());}));
  ...
  const compactLeg=(p,leg,side)=>`<div class="pair-compact-leg"><div class="pair-leg-heading"><span class="${side==='long'?'up':'down'}">${side==='long'?'LONG':'SHORT'}</span><strong>${escapeHTML(p.oi?leg.asset||'—':leg.venue||'미연결')}</strong>${p.oi?`<small class="pair-venue-note">${escapeHTML(leg.venue||'거래소 미정')}</small>`:''}<span class="pair-quantity">${pairNumber(leg.quantity)??'—'} ${escapeHTML(p.oi?'':p.ticker||'')}</span></div><label class="pair-wallet-label"><span>사용 지갑</span><input type="text" data-pair-wallet="${escapeHTML(p.id)}" data-side="${side}" value="${escapeHTML(p[side]?.wallet||venueWallet(leg.venue))}" maxlength="200" placeholder="지갑 이름 또는 주소" aria-label="${escapeHTML(leg.venue||side)} 사용 지갑" autocomplete="off" spellcheck="false"></label></div>`;
  const cardHTML=p=>{const m=pairMetrics(p);return `<article class="pair-card pair-card-compact" ${pairColorAttributes(p)}><div class="pair-card-head"><div><span class="pair-color-label">${p.oi?'OI 채굴':pairColorLabel(p)}</span><h3>${escapeHTML(p.name)}</h3></div><button class="btn" data-pair-edit="${escapeHTML(p.id)}">설정</button></div>${p.oi&&typeof oiPairLive==='function'?oiPairLive(p):''}<div class="pair-compact-legs">${compactLeg(p,m.long,'long')}${compactLeg(p,m.short,'short')}</div><div class="pair-compact-summary"><span>${p.oi?escapeHTML(m.status):`${escapeHTML(p.ticker||'마켓 미설정')} · ${escapeHTML(m.status)}`}</span><strong>순수량 ${m.net===null?'—':Number(m.net.toPrecision(8))}</strong></div><small class="pair-wallet-status" data-wallet-status="${escapeHTML(p.id)}" role="status">지갑 입력 시 자동 저장</small><details class="pair-details" data-pair-details="${escapeHTML(p.id)}" ${expanded.has(p.id)?'open':''}><summary>포인트 · 비용 · 전략 보기 <span>Conviction ${p.conviction||'—'}/5</span></summary><div class="pair-legs">${legHTML(m.long,'long')}${legHTML(m.short,'short')}</div><div class="pair-net"><span>총 명목가치 <b>${m.gross===null?'—':money(m.gross)}</b></span></div><p class="pair-strategy">${escapeHTML(p.strategy||'전략 미작성')}</p><p class="pair-thesis">${escapeHTML(p.reason||'Thesis 미작성')}</p><div class="pair-card-actions"><button class="btn" data-pair-swap="${escapeHTML(p.id)}">Long / Short 전환</button><button class="btn" data-pair-unlink="${escapeHTML(p.id)}">페어 해제</button></div></details></article>`;};
  const oiPairs=pairs.filter(p=>p.oi);
  document.getElementById('pair-board').innerHTML=`${oiPairs.length?`<section class="oi-section"><div class="oi-head"><div><h2>퍼프덱스 OI 채굴 전략 · ${oiPairs.length}</h2><p>상관된 두 자산을 롱/숏으로 묶어 방향 노출 없이 미결제약정(OI)을 쌓습니다.</p></div><button class="btn" data-oi-refresh>${typeof oiFeed!=='undefined'&&oiFeed.loading?'불러오는 중…':'시세 새로고침'}</button></div>${typeof oiFeed!=='undefined'&&oiFeed.error?`<p class="down" role="alert">${escapeHTML(oiFeed.error)}</p>`:''}<div class="pair-grid oi-pair-grid">${oiPairs.map(cardHTML).join('')}</div><p class="oi-foot">7일 상대수익·펀딩: 하이퍼리퀴드 기준 (XYZ100·SP500·Mag7(MAGS)·XAU(GOLD)는 trade.xyz 마켓), XAUT는 OKX 현물.</p></section>`:''}${renderVenueRoster()}<div class="pair-dashboard-heading"><strong>${venueFarmingPairs().length}/${MAX_FARMING_PAIRS} 페어 <span>· ${venueNames.size}/${FARMING_VENUES.length} 거래소</span></strong><button class="btn" onclick="sortBy('conviction')">Conviction ${sortCol==='conviction'?(sortAsc?'↑':'↓'):'↕'}</button></div><div class="pair-grid">${pairs.filter(p=>!p.oi).map(cardHTML).join('')}${!query&&!held?Array.from({length:Math.max(0,MAX_FARMING_PAIRS-venueFarmingPairs().length)},()=>`<div class="pair-empty-slot"><strong>새 페어</strong><span>위에서 거래소 2개를 선택하세요.</span></div>`).join(''):''}</div><details class="pair-board-help"><summary>계산 기준 · 개별 포지션</summary><p class="form-help">선형 PERP의 기초자산 수량 기준입니다. 수량 균형은 청산·거래소·베이시스 위험을 없애지 않습니다. 포인트는 거래소별로 비교하세요.</p><button class="btn" onclick="openPosition('farming')">+ 개별 포지션 기록</button><p class="form-help">지갑 메모는 이 브라우저에 저장되며 계좌 자동 연결과 별개입니다. 기존 개별 포지션은 아래에 보존됩니다.</p></details>${farmingPairs.some(p=>p.archived)?`<details class="pair-archive"><summary>해제한 페어 기록 ${farmingPairs.filter(p=>p.archived).length}개</summary>${farmingPairs.filter(p=>p.archived).map(p=>`<button class="btn" data-pair-edit="${escapeHTML(p.id)}">${escapeHTML(p.name)} · 기록 보기</button>`).join('')}</details>`:''}`;
  document.getElementById('book-title').textContent='DEX PERP · 델타 뉴트럴 페어';
  document.getElementById('book-description').textContent=`OI 채굴 ${activeFarmingPairs().filter(p=>p.oi).length}개 · 거래소 ${venueFarmingPairs().length}/${MAX_FARMING_PAIRS}페어 · ${FARMING_VENUES.length}거래소의 노출 균형과 포인트 효율 관리`;
...
function openFarmingPair(id){
  ...
  document.getElementById('pair-form').addEventListener('submit',event=>{
    ...
    for(const side of ['long','short']){next[side]={...pair[side]};for(const key of ['asset','positionId','venue','wallet','quantity','mark','points7','cost7','asOf','ruleUrl']){const value=values[side+'-'+key]??'';if(key==='asset'&&!pair.oi)continue;next[side][key]=['quantity','mark','points7','cost7'].includes(key)?pairNumber(value):value.trim();}}
...
function validateFarmingPair(pair){
  if(!pair.archived&&!pair.oi){
    if(!venueFarmingPairs().some(p=>p.id===pair.id)&&venueFarmingPairs().length>=MAX_FARMING_PAIRS)return `최대 ${MAX_FARMING_PAIRS}페어까지 만들 수 있습니다.`;
    for(const side of ['long','short']){
      const key=venueKey(pairLeg(pair,side).venue);
      if(key&&venueFarmingPairs().some(p=>p.id!==pair.id&&['long','short'].some(s=>venueKey(pairLeg(p,s).venue)===key)))return '이미 다른 페어에 연결된 거래소입니다.';
    }
  }
