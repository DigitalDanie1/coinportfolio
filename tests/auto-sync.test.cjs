...
run("saveScenarioField('evidence','Scenario private evidence');saveScenarioField('reviewDate','2026-10-01');load();initScenario()");
assert.equal(run('scenario.fields.evidence'),'Scenario private evidence');assert.equal(run('scenario.fields.reviewDate'),'2026-10-01');
assert.equal(run('scenario.text'),'My scenario, invalidation below support');
run("saveScenario(scenario.text)");assert.equal(run('scenario.fields.evidence'),'Scenario private evidence');
run("saveScenarioField('view','<img src=x onerror=alert(1)>');renderScenarioFramework()");
assert(!el('scenario-framework').innerHTML.includes('<img src=x'));assert(el('scenario-framework').innerHTML.includes('&lt;img'));
...
assert(el('auto-status').textContent.includes('자동 갱신 켜짐'));assert(!JSON.stringify(urls).includes('Never overwrite'));assert(!JSON.stringify(urls).includes('avgPrice'));assert(!JSON.stringify(urls).includes('My scenario'));assert(!JSON.stringify(urls).includes('Scenario private evidence'));assert(!JSON.stringify(urls).includes('My private reflection'));assert.equal(run('reflections.text'),'My private reflection');assert.equal(run('holdings.uni.conviction'),5);assert.equal(run('coinData(coins.find(c=>c.id==="uni")).price'),9);
